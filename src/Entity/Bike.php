...
class Bike
{
    ...
    /**
     * @ORM\OneToOne(targetEntity="App\Entity\BikeOwner", inversedBy="bike", cascade={"persist", "remove"})
     */
    private $bikeOwner;
    ...
    public function getStatusTransKey(): ?string
    {
        ...
        if ($status == 1)
        {
            return 'bike.form.status.available.label';
        }
        elseif ($status == 2){
            return 'bike.form.status.unavailable.label';
        }
        elseif ($status == 3){
            return 'bike.form.status.repair.label';
        }
        else{
            return 'bike.form.status.default.label';
        }
    }
